...
@router.get("", response_model=BaseResponse[QueryHistoryListResponse], summary="获取查询历史列表")
async def list_query_histories(
    ...
    """获取查询历史列表（分页），支持搜索和筛选"""
    result = await service.get_history_list(query, operation_context=operation_context)
    return BaseResponse(data=result)


@router.get("/{history_id}", response_model=BaseResponse[QueryHistoryDetailResponse], summary="获取查询历史详情")
async def get_query_history(
    ...
    result = QueryHistoryDetailResponse(**history.model_dump())
    return BaseResponse(data=result)


@router.post("", response_model=BaseResponse[QueryHistoryResponse], status_code=status.HTTP_201_CREATED, summary="创建查询历史")
...
@router.post("/batch", response_model=BaseResponse[list[QueryHistoryResponse]], summary="批量创建查询历史")
async def batch_create_query_histories(
    ...
    """批量创建查询历史记录"""
    result = await service.batch_create_histories(histories_data, operation_context)
    return BaseResponse(data=result)
...
@router.get("/recent", response_model=BaseResponse[list[QueryHistoryResponse]], summary="获取最近查询历史")
...
@router.get("/statistics", response_model=BaseResponse[dict], summary="获取查询历史统计")
async def get_query_history_statistics(
    ...
    """获取查询历史统计信息"""
    result = {"total_queries": 0, "user_queries": 0, "device_queries": 0, "query_types": {}, "daily_stats": []}
    return BaseResponse(data=result)
